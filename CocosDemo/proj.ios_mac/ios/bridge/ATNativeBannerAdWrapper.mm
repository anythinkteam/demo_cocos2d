...
-(void) didShowNativeBannerAdInView:(ATNativeBannerView*)bannerView placementID:(NSString*)placementID {
    ...
    if (callback != NULL) {
        ATCocosNativeBannerAdListener* pDelegate = (ATCocosNativeBannerAdListener*)callback;
        const char* cPlacementId = [ATCocosUtils cstringFromNSString:placementID];
        pDelegate->onNativeBannerShow(cPlacementId);
    }
}
...
//    [self invokeCallback:@"OnNativeBannerAdClick" placementID:placementID error:nil extra:nil];
    ...
    if (callback != NULL) {
        ATCocosNativeBannerAdListener* pDelegate = (ATCocosNativeBannerAdListener*)callback;
        const char* cPlacementId = [ATCocosUtils cstringFromNSString:placementID];
        pDelegate->onNativeBannerClicked(cPlacementId);
    }
}
...
//    [self invokeCallback:@"OnNativeBannerAdCloseButtonClicked" placementID:placementID error:nil extra:nil];
    ...
    if (callback != NULL) {
        ATCocosNativeBannerAdListener* pDelegate = (ATCocosNativeBannerAdListener*)callback;
        const char* cPlacementId = [ATCocosUtils cstringFromNSString:placementID];
        pDelegate->onNativeBannerClose(cPlacementId);
    }
}
...
//    [self invokeCallback:@"OnNativeBannerAdAutorefresh" placementID:placementID error:nil extra:nil];
    ...
    if (callback != NULL) {
        ATCocosNativeBannerAdListener* pDelegate = (ATCocosNativeBannerAdListener*)callback;
        const char* cPlacementId = [ATCocosUtils cstringFromNSString:placementID];
        pDelegate->onNativeBannerAutoRefresh(cPlacementId);
    }
}
...
//    error = error != nil ? error : [NSError errorWithDomain:@"com.AT.Unity3DPackage" code:100001 userInfo:@{NSLocalizedDescriptionKey:@"AT has failed to refresh native banner ad", NSLocalizedFailureReasonErrorKey:@"AT has failed to refresh native banner ad"}];
//    [self invokeCallback:@"OnNativeBannerAdAutorefreshFailed" placementID:placementID error:error extra:nil];
    ...
    if (callback != NULL) {
        ...
        pDelegate->onNativeBannerAutoRefreshFail(cPlacementId, errorDict.jsonString.UTF8String);
    }
}
@end
